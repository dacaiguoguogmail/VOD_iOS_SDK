...
@interface DWNetworkMonitor ()<SimplePingDelegate>
...
@implementation DWNetworkMonitor

-(instancetype)initWithPingHostName:(NSString *)hostName
{
    if (self == [super init]) {
...
-(void)setMaxPacketNum:(NSInteger)maxPacketNum
{
    if (_pinging) {
        NSLog(@"ping 中，请先调用stopPing方法");
        return;
    }
...
-(void)startPing
{
    if (!self.pinger || !self.hostName) {
        NSLog(@"请调用 initWithPingHostName 方法初始化");
        return;
    }
    
    self.runloopRef = CFRunLoopGetCurrent();
    
    //开始ping前，清空下数据，防止再次start包 没有清空
...
-(void)sendPing
{
    if (self.packets.count < self.maxPacketNum) {
       
        [self.pinger sendPingWithData:nil];
    }
    
    if (self.packets.count == self.maxPacketNum) {
        //到达最大发送包数时，启动一个超时的计时器
        ...
        dispatch_once(&onceToken, ^{
            ...
            self.timeOutTimer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(timeOutAction) userInfo:nil repeats:YES];
        });
    }
}
...
//发送新包，创建item
-(void)createNewPacketItem:(uint16_t)sequenceNumber AndPacketBytes:(NSInteger)packetBytes
{
    DWNetworkMonitoringItem * packetItem = [[DWNetworkMonitoringItem alloc] init];
    packetItem.sequenceNumber = sequenceNumber;
    packetItem.sendPacketTime = [[NSDate date] timeIntervalSince1970] * 1000;
    packetItem.packetBytes = packetBytes;
    packetItem.addressIP = [self displayAddressForAddress:self.pinger.hostAddress];
    [self.packets addObject:packetItem];
}

#pragma mark - SimplePingDelegate
-(void)simplePing:(SimplePing *)pinger didStartWithAddress:(NSData *)address
{
    NSLog(@"%s %@",__func__,[self displayAddressForAddress:address]);
...
{
//    NSLog(@"%s didFailWithError:%@",__func__,error);
...
{
//    NSLog(@"%s send:%u",__func__,sequenceNumber);
    
    [self createNewPacketItem:sequenceNumber AndPacketBytes:packet.length];
}
...
{
//    NSLog(@"%s %u received,size: %ld",__func__,sequenceNumber,packet.length);
    
    //收到 保存当前时间 计算 发送到收到 使用的时间
    //找到发送的packet
//    __weak typeof(self) weakSelf = self;
    NSTimeInterval receiveTime = [[NSDate date] timeIntervalSince1970] * 1000;
    [self.packets enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        DWNetworkMonitoringItem * packetItem = (DWNetworkMonitoringItem *)obj;
        if (packetItem.sequenceNumber == sequenceNumber) {
            *stop = YES;
            
            packetItem.receivePacketTime = receiveTime;
            
        }
        
    }];
    
    NSLog(@"didReceivePingResponsePacket : %hu",sequenceNumber);
    if (sequenceNumber == self.maxPacketNum - 1) {
        [self stop];
    }
}
...
{
    NSLog(@"%s unexpected packet, size=%zu",__func__ ,packet.length);
//    [self shouldStopPing];
}
...
-(NSString *)displayAddressForAddress:(NSData *)address
{
    ...
    int succesee = getnameinfo(address.bytes, sizeof(address), host, sizeof(host), service, sizeof(service), NI_NUMERICSERV);
    if (succesee != 0) {
        NSLog(@"%s 失败",__func__);
        return @"?";
    }
...
@implementation DWNetworkMonitoringItem

//计算网络延迟
-(double)networkDelay
{
    if (!self.receivePacketTime || self.receivePacketTime == 0) {
        return -1;
    }
    
    return (self.receivePacketTime - self.sendPacketTime) / 2.0;
}
