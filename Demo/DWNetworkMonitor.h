...
typedef void(^DWNetworkMonitoringFinishBlock)(NSArray *packets);

@interface DWNetworkMonitor : NSObject
...
//最大发送packet 数量 默认4个
@property(nonatomic,assign)NSInteger maxPacketNum;

@property(nonatomic,readonly)BOOL isPinging;

/**
 完成ping回调*/
@property(nonatomic,copy)DWNetworkMonitoringFinishBlock finishBlock;
...
-(void)startPing;
/**
 结束ping 不需要显示调用stop,ping到最大的包数会自动停止 回调block
 如果在子线程中ping,会结束当前子线程
 */
-(void)stopPing;
...
@interface DWNetworkMonitoringItem : NSObject

//packet 序号
@property(nonatomic,assign) uint16_t sequenceNumber;
//访问的ip地址
@property(nonatomic,strong) NSString * addressIP;
//packet 大小，默认64字节
@property(nonatomic,assign) NSInteger packetBytes;
//发送packet时间
@property(nonatomic,assign) NSTimeInterval sendPacketTime;
//接收到packet时间
@property(nonatomic,assign) NSTimeInterval receivePacketTime;
//网络延迟 ms ，如果为-1 证明丢包
@property(nonatomic,assign) double networkDelay;
